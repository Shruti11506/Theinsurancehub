...
export default function BrandIntroSplash({ onComplete }) {
  ...
  // Exact target calculation to target DOM elements
  ...
  useEffect(() => {
    ...
    // Stay centered together from start, then move begins at 2.2s
  ...
  const startMove = () => {
    ...
    // Move duration 1.35s (2.2s + 1.35s = 3.55s total)
    setTimeout(() => {
      setPhase('done');
      ...
      <div className="relative z-20 flex flex-col items-center text-center px-6 max-w-4xl pointer-events-none">
        ...
        >
          <img
            ...
            className="h-20 sm:h-24 w-auto object-contain mix-blend-multiply drop-shadow-sm"
          />
        </motion.div>
        ...
        <motion.h1
          ...
          className="text-3xl sm:text-4xl lg:text-5xl xl:text-6xl font-extrabold tracking-tight leading-tight font-serif bg-gradient-to-r from-insurance-darkblue to-insurance-orange bg-clip-text text-transparent pb-2 uppercase"
          style={{ transformOrigin: 'top left', willChange: 'transform' }}
        >
          Confused about choosing the <br className="hidden md:inline" />
          right insurance?
        </motion.h1>
        ...
        >
          {/* Answer */}
          <p className="text-xl sm:text-2xl lg:text-3xl font-bold text-black leading-relaxed font-sans border-l-4 border-insurance-orange pl-5">
            All companies, insurance and mutual funds <br className="hidden sm:inline" />
            under one roof.
          </p>

          {/* Italic Quote */}
          <p className="text-[17px] sm:text-[19px] italic font-medium text-slate-500 pl-1 border-l-2 border-insurance-darkblue/30 tracking-wide font-serif">
